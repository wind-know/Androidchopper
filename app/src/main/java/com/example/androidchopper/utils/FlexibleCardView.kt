    ...
    private fun updateConstraintsByMode(mode: Mode) {
        ...
        when (mode) {
            Mode.CARD_ICON_TEXT -> {
                ...
                // 文字在图标下方
                constraintSet.connect(singleTextTv.id, ConstraintSet.TOP, iconIv.id, ConstraintSet.BOTTOM, 4.dpToPx())
            ...
            Mode.LEFT_IMAGE_RIGHT_TEXT -> {
                ...
                // 图标居左，高度撑满
                constraintSet.connect(iconIv.id, ConstraintSet.START, ConstraintSet.PARENT_ID, ConstraintSet.START, 8.dpToPx())
                ...
                constraintSet.constrainWidth(iconIv.id, (smallRectHeightDp * 0.8).dpToPx())
